<!-- GENERATED from the operation registry (core/src/opw/operations.ts) — do not edit by hand. -->

# Lynx PDF Studio — Operations Reference

**100 operations.** A workflow (`workflow.opw.yaml`) lists `inputs`, an ordered `operations:` program, and an `output`. Each operation below is one entry in that list.
...
Combines the workflow's `inputs` into one PDF. Either list files in order (`inputs: [a.pdf, b.pdf]`), or point at a whole folder with a glob (`inputs: [docs/*.pdf]`) to merge every matching file in **sorted filename order** — that's why zero-padded names (part_000001.pdf) matter for a re-merge.
...
- **params:** `interleave`, `convert`
- **also written as:** `interleave`
...
##### A whole folder, whatever is in it
...
##### Strict: refuse anything that isn't already a PDF
...
- **params:** `ranges`, `every`, `max_size`, `min_pages`, `name`
- **also written as:** `burst`
...
##### Every part under an upload limit
...
##### Name the parts for a build pipeline
...
- **also written as:** `burst_invoices`, `split_receipts`
...
- **params:** `pages*`, `after`, `before`
- **inputs:** contract.pdf → **output:** `output/contract-reordered.pdf`
- **operation:** `move_pages: { pages: [2, "5-9"], after: 50 }`
...
- **params:** `a*`, `b*`
- **inputs:** report.pdf → **output:** `output/report-fixed.pdf`
- **operation:** `swap_pages: { a: 2, b: 10 }`
...
Inserts a blank page before the 1-based position `at`. `size` copies a neighbor by default ("match"), or takes "A4", "Letter" or "Legal".
...
- **params:** `title`, `subtitle`, `author`, `date`, `at`, `size`, `font`, `title_size`, `margin`, `color`, `background`, `image`, `image_width`, `align`, `rule`
- **also written as:** `cover_page`, `cover`
- **inputs:** quarterly-report.pdf → **output:** `output/quarterly-report-covered.pdf`
- **operation:** `title_page: { title: "Quarterly Report", subtitle: "FY2026 · Q3", author: "Finance" }`
...
##### The e-reader fix — cover page plus metadata
...
##### A cover in any language, from Markdown
...
##### Section dividers — insert back to front
...
- **also written as:** `annotations`, `markup`
...
- **params:** `text`, `patterns`, `regex`, `style`, `color`, `opacity`, `width`, `ignore_case`, `whole_word`, `pages`, `note`, `author`, `flatten`, `preview`, `to`
- **also written as:** `mark_text`, `highlight_text`, `find_highlight`
- **inputs:** contract.pdf → **output:** `output/contract-marked.pdf`
- **operation:** `highlight: { text: ["Confidential", "Net 30"], style: highlight, color: "#ffd400", ignore_case: true }`
...
- **also written as:** `header`, `footer`, `bates_number`, `bates`
...
- **also written as:** `extract_comments`, `extract_markup`
...
- **params:** `to`, `format`
- **also written as:** `accessibility_check`, `a11y_check`
...
- **params:** `title`, `lang`, `alt`, `force`
- **also written as:** `autotag`, `auto_tag`, `add_tags`
- **inputs:** untagged-report.pdf → **output:** `output/tagged.pdf`
- **operation:** `tag_pdf: { lang: "en-US", title: "Q3 Report", alt: ["Bar chart of quarterly revenue"] }`
...
- **also written as:** `diff_pdfs`, `pdf_compare`
...
- **params:** `to`, `dpi`, `format`, `pages`, `transparent`, `name`, `text_as_path`
...
- **params:** `find*`, `replace`, `regex`, `ignore_case`, `whole_word`, `pages`, `font`, `font_file`, `preview`, `to`
- **also written as:** `find_replace_text`, `find_and_replace`
...
- **also written as:** `receipt_extract`, `extract_receipts`
...
- **also written as:** `make_fillable`, `add_form_fields`
...
- **also written as:** `form_to_json`, `extract_form_data`
...
- **also written as:** `extract_urls`, `extract_hyperlinks`, `list_links`
...
- **also written as:** `linkify`, `make_links`
...
- **params:** `quality`, `max_size`
- **also written as:** `compress_images`
...
##### Get under an email or upload limit
...
##### Compress first, then split what's left
...
- **params:** `mode`
- **also written as:** `convert_color`, `color_convert`
...
##### Every deck in a folder → its own PDF
...
##### Every deck in a folder → ONE merged PDF
...
##### A folder of decks → printable handouts
...
- **params:** `every`, `max_frames`, `width`, `timestamps`
- **also written as:** `contact_sheet`, `storyboard`
- **inputs:** lecture.mp4 → **output:** `output/lecture-frames.pdf`
- **operation:** `video_to_pdf: { every: 30 }`
- **result:** Samples the recording every 30 seconds, one timestamped frame per page.

#### More workflows

##### A reviewable contact sheet
...
- **params:** `to`, `dpi`, `pages`, `transparent`, `name`
- **inputs:** contract.pdf → **output:** `output/contract.pdf`
- **operation:** `pdf_to_png: { to: output/png, dpi: 300 }`
- **result:** Writes one 300-DPI PNG per page into output/png/ — print-quality images of every page.

#### More workflows

##### Just the cover, as a thumbnail
...
##### Transparent PNGs, named so they sort
...
- **params:** `to`, `dpi`, `pages`, `name`
- **also written as:** `pdf_to_jpeg`
- **inputs:** photo-book.pdf → **output:** `output/photo-book.pdf`
- **operation:** `pdf_to_jpg: { to: output/jpg, dpi: 150 }`
